...
-- ============================================================================
-- Table 4: weather_embeddings
-- Vector embeddings for semantic search over weather data
...
CREATE TABLE IF NOT EXISTS weather_embeddings (
    ...
    severity VARCHAR(50),
    search_type VARCHAR(50),  -- 'weather_events', 'climate_data', or 'forecasts'
    metadata JSONB,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

COMMENT ON TABLE weather_embeddings IS 'Vector embeddings for semantic weather data search';
COMMENT ON COLUMN weather_embeddings.event_type IS 'Type of weather event (e.g. thunderstorm, heatwave)';
COMMENT ON COLUMN weather_embeddings.description IS 'Natural language description of the weather data';
COMMENT ON COLUMN weather_embeddings.embedding IS 'Vector embedding (384-dim) generated by sentence-transformers';
COMMENT ON COLUMN weather_embeddings.search_type IS 'Category for filtering: weather_events, climate_data, or forecasts';
COMMENT ON COLUMN weather_embeddings.metadata IS 'Additional JSON metadata (lat/long, temperature, etc.)';
...
CREATE INDEX IF NOT EXISTS weather_embeddings_search_type_idx 
ON weather_embeddings(search_type);

CREATE INDEX IF NOT EXISTS weather_embeddings_date_idx 
ON weather_embeddings(date);
